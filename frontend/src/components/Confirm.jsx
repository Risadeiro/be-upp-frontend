        ...
        <div className="mainContainer" key={questionId}>
          ...
          {questionInfo.value
            ? Object.entries(questionInfo.value).map(([itemId, itemInfo]) => (
                <h3 className="answer" key={`answer-${itemId}-${questionId}`}>
                  {" "}
                  {itemInfo}{" "}
                </h3>
              ))
    ...
    case "scale":
      return (
        <div className="mainContainer" key={questionId}>
          ...
          <h3 className="answer" key={`answer-${questionId}`}>
            {" "}
            {questionInfo.value}{" "}
          </h3>
        </div>
    ...
    case "radio":
      return (
        <div className="mainContainer" key={questionId}>
          ...
          {questionInfo.value
            ? Object.entries(questionInfo.value).map(([, itemInfo]) => (
                <h3 className="answer" key={`answer-${questionId}`}>
                  {" "}
                  {itemInfo}{" "}
                </h3>
      ...
      return (
        <div className="mainContainer" key={questionId}>
          <FormLabel
            key={questionId}
            component="legend"
            style={{fontSize: 20, flex: 2, marginBottom: 20}}
          >
            {dict[questionId][1]}
          </FormLabel>
          <div className="break" />
          ...
          {questionInfo.value
            ? Object.entries(questionInfo.value).map(([itemId, itemInfo]) => (
                <React.Fragment key={`${itemId}-${questionId}`}>
                  ...
                  <h3 className="answer" key={`3-${questionId}`}>
                    {" "}
                    {itemInfo.colLabel}{" "}
                  </h3>
                  <div className="line" />
